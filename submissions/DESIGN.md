...
| **`chat_id` not managed in MCP tool** | The MCP tool is stateless — it runs the pipeline and returns with no memory between calls. The Streamlit UI owns the session lifecycle and threads `chat_id` automatically via `st.session_state`; the MCP tool has no equivalent. The caller must pass `chat_session_id` explicitly on each turn if multi-turn continuity is needed. |
