...
| **Non-streaming Chat API / response latency** | Every question makes two sequential API calls — Search then Chat — so end-to-end latency is their sum. The sandbox does not support streaming, so the UI blocks until the full Chat response is ready. The `st.status()` progress indicator manages perceived latency but not actual wait time. In production, switching to `create_stream()` with SSE would allow the answer to render incrementally. |
...
| **Session management differs by interface** | The Streamlit UI owns the full session lifecycle: `chat_id` is initialized in `st.session_state`, threaded automatically on every chat call, updated with the returned ID after each turn, and cleared when the datasource changes to prevent context bleed. The MCP tool is stateless by design — it runs the pipeline and returns with no memory between calls, so the caller must pass `chat_session_id` explicitly on each turn if multi-turn continuity is needed. This is a deliberate tradeoff: stateless tools are simpler and composable across any client, while Streamlit manages state because it owns the session. |
...
| **Markdown-only document ingestion** | The indexer reads only `.md` files, which keeps the ingestion path simple and dependency-free. The Glean Indexing API is format-agnostic — it accepts any text with a `mimeType` — so extending to PDFs or HTML is straightforward. For this prototype, Markdown is sufficient: documents from Google Docs, Notion, or Confluence can be exported as `.md` without loss of indexable content, and the lower parsing overhead means faster indexing cycles during iteration. |
